...
FROM 
    client 
WHERE 
    client_join_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)""")
clients = cursor.fetchall()

print("-- Clients Enrolled in Past 6 Months --")
for count, (first_name, last_name, email, street, city, state, zip_code, phone_number, join_date) in enumerate(clients, start=1):
    print(f"Client {count}:")
    print(f"Name: {first_name} {last_name}")
    print(f"Email: {email}")
    print(f"Address: {street}, {city}, {state}, {zip_code}")
    print(f"Phone: {phone_number}")
    print(f"Enrollment Date: {join_date.strftime('%Y-%m-%d')}")
    print() 
